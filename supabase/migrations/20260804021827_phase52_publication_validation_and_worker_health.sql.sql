-- ============================================================
-- Phase 5.2: Publication Validation, Provider Safety, Worker Health
-- ============================================================

-- 1. Worker health tracking table
...
CREATE POLICY "worker_health_super_admin_select" ON worker_health_log
  FOR SELECT TO authenticated
  USING (is_super_admin());

CREATE POLICY "worker_health_service_insert" ON worker_health_log
  FOR INSERT TO service_role
  WITH CHECK (true);

-- Only service_role can insert; no anon/authenticated insert
REVOKE ALL ON worker_health_log FROM anon, authenticated;
GRANT SELECT ON worker_health_log TO authenticated;

-- 2. Add test_mode column to automation_workflows
ALTER TABLE automation_workflows
  ADD COLUMN IF NOT EXISTS test_mode boolean NOT NULL DEFAULT false;

-- 3. Add provider_status column to communication_messages for readiness tracking
ALTER TABLE communication_messages
  ADD COLUMN IF NOT EXISTS provider_ready boolean DEFAULT true;

-- ============================================================
-- 4. Replace publish_workflow_version with full validation
-- ============================================================
  ...
  FOR v_node IN SELECT * FROM jsonb_array_elements(v_nodes) LOOP
    ...
    IF v_action_type = 'send_email' THEN
      ...
      IF v_template_id IS NULL THEN
        v_errors := array_append(v_errors, 'Node ' || v_node_id || ' (send_email): template_id is required');
      ELSE
    ...
    IF v_action_type = 'send_sms' THEN
      v_template_id := NULLIF(v_config->>'template_id', '')::uuid;
      IF v_template_id IS NULL THEN
        v_errors := array_append(v_errors, 'Node ' || v_node_id || ' (send_sms): template_id is required');
      ELSE
        SELECT channel, tenant_id, is_active INTO v_template_channel, v_template_tenant, v_template_active
        FROM communication_templates WHERE id = v_template_id;
        IF v_template_tenant IS NULL THEN
    ...
    -- send_whatsapp validation
    IF v_action_type = 'send_whatsapp' THEN
      v_template_id := NULLIF(v_config->>'template_id', '')::uuid;
      IF v_template_id IS NULL THEN
        v_errors := array_append(v_errors, 'Node ' || v_node_id || ' (send_whatsapp): template_id is required');
      ELSE
        SELECT channel, tenant_id, is_active INTO v_template_channel, v_template_tenant, v_template_active
        FROM communication_templates WHERE id = v_template_id;
        IF v_template_tenant IS NULL THEN
    ...
    IF v_action_type IN ('if_else', 'condition') THEN
      ...
      END IF;
      -- if_else must have true and false branches
...
-- Revoke execute from anon and authenticated; only service_role and the SECURITY DEFINER path
REVOKE EXECUTE ON FUNCTION publish_workflow_version(uuid, jsonb, text, uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION publish_workflow_version(uuid, jsonb, text, uuid) TO authenticated;

-- ============================================================
-- 5. Provider readiness check function
-- ============================================================
...
BEGIN
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND (capabilities->>p_channel) = 'true'
...
REVOKE EXECUTE ON FUNCTION check_provider_readiness(uuid, text) FROM anon;
GRANT EXECUTE ON FUNCTION check_provider_readiness(uuid, text) TO service_role;

-- ============================================================
-- 6. Log worker invocation function
-- ============================================================
CREATE OR REPLACE FUNCTION public.log_worker_invocation(
  p_worker_name text,
  p_success boolean,
  p_processed_count int DEFAULT 0,
  p_failed_count int DEFAULT 0,
  p_dead_letter_count int DEFAULT 0,
...
REVOKE EXECUTE ON FUNCTION log_worker_invocation(text, boolean, int, int, int, int, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION log_worker_invocation(text, boolean, int, int, int, int, text) TO service_role;
