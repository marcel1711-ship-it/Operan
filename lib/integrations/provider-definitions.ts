...
export const PROVIDER_DEFINITIONS: ProviderDefinition[] = [
    ...
    credentials: [
      {
        ...
        purpose: 'Identifies the OPERAN Twilio account for SMS messaging.',
        classification: 'server_only',
        required: true,
        where_to_obtain: 'Twilio Console → Account → Account Info → Account SID',
        where_to_configure: 'Vercel → Project → Settings → Environment Variables',
        redeploy_required: true,
      },
      {
        ...
        purpose: 'Authenticates API requests to the Twilio messaging service.',
        classification: 'server_only',
        required: true,
        where_to_obtain: 'Twilio Console → Account → Account Info → Auth Token',
        where_to_configure: 'Vercel → Project → Settings → Environment Variables',
        redeploy_required: true,
      },
    ],
    healthChecks: [
      { label: 'Twilio Account SID', hint: 'Set TWILIO_ACCOUNT_SID in environment variables' },
      { label: 'Twilio Auth Token', hint: 'Set TWILIO_AUTH_TOKEN in environment variables' },
    ],
    callbackUrls: [],
    webhookUrls: [],
    tenantCapabilities: [
      { label: 'Configure Sender Number', description: 'Tenants select a Twilio phone number for outbound SMS.' },
      { label: 'Send SMS Notifications', description: 'Automated SMS for booking confirmations and reminders.' },
    ],
    setupInstructions: [
      'Twilio SMS integration is currently in development.',
  ...
  // ── Provider 10: Twilio WhatsApp ──
  {
    ...
    credentials: [
      {
        env_var: 'TWILIO_ACCOUNT_SID',
        label: 'Twilio Account SID',
        purpose: 'Identifies the OPERAN Twilio account for WhatsApp messaging.',
        classification: 'server_only',
        required: true,
        where_to_obtain: 'Twilio Console → Account → Account Info → Account SID',
        where_to_configure: 'Vercel → Project → Settings → Environment Variables',
      ...
      },
      {
        env_var: 'TWILIO_AUTH_TOKEN',
        label: 'Twilio Auth Token',
        purpose: 'Authenticates API requests to the Twilio WhatsApp API.',
        classification: 'server_only',
        required: true,
        where_to_obtain: 'Twilio Console → Account → Account Info → Auth Token',
        where_to_configure: 'Vercel → Project → Settings → Environment Variables',
    ...
    webhookUrls: [],
    tenantCapabilities: [
      { label: 'Configure WhatsApp Sender', description: 'Tenants configure a Twilio WhatsApp sender number.' },
      { label: 'Send WhatsApp Messages', description: 'Automated WhatsApp notifications for bookings.' },
    ],
    setupInstructions: [
      'WhatsApp (Twilio) integration is currently in development.',
      'The backend adapter and tenant connection flow are not yet available.',
      'No configuration is required at this time.',
    ],
  },
